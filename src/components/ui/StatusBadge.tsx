...
import { CheckCircle, Car, Wrench, Clock, type LucideIcon } from 'lucide-react-native';
...
export function StatusBadge({ status, size = 'md', className }: StatusBadgeProps) {
  ...
  const label = translated === config.translationKey ? config.fallbackLabel : translated;

  return (
    <View style={containerStyle} className={className}>
      <Icon size={iconSize} color={config.color} />
      <Text
        variant={isSmall ? 'labelSmall' : 'bodySmall'}
        color={config.color}
      >
        {label}
      </Text>
